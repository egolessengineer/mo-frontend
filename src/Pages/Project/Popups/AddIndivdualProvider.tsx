...
const AddIndivdualProvider = ({
  ...
  const [selectedIps, setSelectedIps] = useState(
    indexFormik.values.individualProvider
  );
  ...
  async function handleConfirm() {
    try {
      ...
      selectedIps && selectedIps.forEach((element: any) => {

        providerId.push(element?.userId)
      ...
      await MODIFY_IP(body);
      await silentRefetch()
      await indexFormik.setFieldValue("individualProvider", selectedIps);
      close();
    } catch (error) {
      handleCustomError(error);
    }
  ...
  const isAdded = (targetId: string) => {
    return selectedIps.some((obj: any) => obj.userId === targetId);
  };

  const RemoveIndividualProvider = (targetId: string) => {
    const ips = selectedIps.filter((obj: any) => obj.userId !== targetId);
    setSelectedIps(ips);
  };
                        ...
                        >
                          ...
                          <div className="flex flex-col pl-[13px] pt-[9px] w-[100%]">
                            ...
                            <div className="flex">
                              ...
                              />
                              <Typography
                                label={Element?.id || Element?.Member?.id}
                                type="p"
                                FontSize="xs"
                                color="primary"
                                variant={300}
                          ...
                          <div className="flex justify-end items-center m-[20px]">
                            <div className="w-[131px] flex gap-2">
                              <Button
                                onClick={() => {
                                  const selectedUserId = Element?.id;
                                  const isUserAdded = isAdded(selectedUserId);

                                  if (isUserAdded) {
                                    RemoveIndividualProvider(selectedUserId);
                                  } else {
                                    const newSelectedIps = [
                                      ...selectedIps,
                                      {
                                        userId: Element?.id,
                                        name: Element?.name,
                                        position:
                                          Element?.Experiences &&
                                          Element.Experiences.length > 0 &&
                                          Element.Experiences[
                                            Element.Experiences.length - 1
                                          ]?.position,
                                        profilePictureLink:
                                          Element?.About?.profilePictureLink,
                                      },
                                    ];
                                    setSelectedIps(newSelectedIps);
                                  }
                                }}
                                label={isAdded(Element?.id) ? "Remove" : "Add"}
                                variant="Transparent"
                                color="secondary"
                                size="small"
                                className=""
            ...
            <div className="lg:w-[30%] w-full mt-[10px]">
              ...
              <div className="flex flex-col gap-5 ">
                ...
                <div className="md:w-[366px]  shadow-navbar">
                  <div className="m-[10px] border-[1px] border-text-gray-50 md:w-[346px] h-[176px] rounded-[5px] overflow-y-auto">
                    {selectedIps &&
                      selectedIps.length > 0 &&
                      selectedIps.map((data: any, idx: any) => (
                        <div
                          key={idx}
                          className="flex flex-wrap gap-x-[10px] p-[10px] "
                        >
                          <Typography
                            label={data?.name}
                            type="p"
                          ...
                          <HelperText
                            ...
                            onClick={() =>
                              RemoveIndividualProvider(data.userId)
                            }
                            label=""
                            icon={
                              <img
                  ...
                  <div className="flex justify-end mr-[20px] ">
                    <Button
                      label=" Clear list "
                      onClick={() =>
                        indexFormik.setFieldValue("individualProvider", [])
                      }
                      variant="line"
                      size="small"
...
export default AddIndivdualProvider;
